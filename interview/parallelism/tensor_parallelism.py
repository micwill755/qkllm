...
class TensorParallelMLPBlock:
    def __init__(self, fc1_weights, fc1_bias, fc2_weights, fc2_bias, tp_size=2):
        ...
        # split the second dimension as this is the hidden dimension
        hidden_shard_size = len(fc1_weights[-1]) // tp_size # 32 / 2 = 16

        self.w1 = []
        self.b1 = []

        # split fc1 by columns
        for i in range(tp_size):
            start_idx = i * hidden_shard_size
            end_idx = start_idx + hidden_shard_size
        ...
        for i in range(tp_size):
            start_idx = i * hidden_shard_size
            end_idx = start_idx + hidden_shard_size
            self.w2.append(fc2_weights[start_idx:end_idx, :])
...
if __name__ == "__main__":  
    # --- 1. Set Parameters ---
    B, C_in, C_hidden, C_out = 1, 8, 32, 8
    learning_rate = 0.01
    TP_SIZE = 2
    np.random.seed(42)
