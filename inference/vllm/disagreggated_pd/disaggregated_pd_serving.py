...
class ProxyServer:
    ...
    def run_server(self):
        print(f"🚀 Starting disaggregated proxy server on port {self.port}")
        print(f"📡 Prefill instances: {self.proxy_instance.prefill_instances}")
        print(f"🔄 Decode instances: {self.proxy_instance.decode_instances}")
        
        app = FastAPI()
        app.include_router(self.proxy_instance.router)
        config = uvicorn.Config(app, port=self.port, loop="uvloop", log_level="info")
        server = uvicorn.Server(config)
        
        print(f"✅ Proxy server ready at http://localhost:{self.port}")
        server.run()
